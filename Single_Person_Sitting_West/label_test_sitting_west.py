...
image_folder = Path("/Users/saklaen/Downloads/IR_Samples/Single_Person_Sitting_West/Single_Person_Sitting_West_image")
...
labels_file = Path(__file__).parent / "Single_Person_Sitting_West"
...
if labels_file.exists():
    ...
    current_index = len(data.get("Single_Person_Sitting_West", {}))
...
def save_label():
    ...
    if not labels_file.exists():
        data = {
            "Single_Person_Sitting_West": {}
        }
    else:
        with open(labels_file, "r") as f:
            data = json.load(f)

        data.setdefault("Single_Person_Sitting_West", {})

    data["Single_Person_Sitting_West"][image_name] = matrix

    with open(labels_file, "w") as f:
        json.dump(data, f, indent=4)

    print(f"{image_name} saved!")
    print("Sitting West count:", len(data["Single_Person_Sitting_West"]))
...
image_name_label = tk.Label(
    control_frame,
    text="Image: Single_Sitting_West_0001",
    font=("Arial", 14, "bold")
)
