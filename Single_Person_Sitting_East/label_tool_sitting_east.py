...
image_folder = Path("/Users/saklaen/Downloads/IR_Samples/Single_Person_Sitting_East/Single_Person_Sitting_East_image")
...
labels_file = Path(__file__).parent / "Single_Person_Sitting_East"
...
if labels_file.exists():
    ...
    current_index = len(data.get("Single_Person_Sitting_East", {}))
...
def save_label():
    ...
    if not labels_file.exists():
        data = {
            "Single_Person_Sitting_East": {}
        }
    else:
        with open(labels_file, "r") as f:
            data = json.load(f)

        data.setdefault("Single_Person_Sitting_East", {})

    data["Single_Person_Sitting_East"][image_name] = matrix

    with open(labels_file, "w") as f:
        json.dump(data, f, indent=4)

    print(f"{image_name} saved!")
    print("Sitting East count:", len(data["Single_Person_Sitting_East"]))
...
image_name_label = tk.Label(
    control_frame,
    text="Image: Single_Sitting_East_0001",
    font=("Arial", 14, "bold")
)
